...
export async function promptTokenInteractive(server: string): Promise<string> {
	// Open the dedicated CLI Access app path (not /admin): JWT audience must
	// match `Perseus Admin CLI`.
	const accessApp = accessAppFor(server);
	console.log(`
────────────────────────────────────────────────────────────
...
/**
 * Probe whether Access accepts this JWT through the dedicated CLI alias.
...
export async function probeAccessToken(
	...
	try {
		const accessUrl = accessAppFor(server);
		...
		if (res.status >= 500) {
			console.warn(
				`probeAccessToken: ${accessUrl} returned ${res.status} — ` +
 ...
 * / CF-Access-Client-Secret) through the dedicated CLI alias. Mirrors
...
export async function probeServiceToken(
	...
	try {
		...
		if (res.status >= 500) {
			console.warn(
				`probeServiceToken: ${accessUrl} returned ${res.status} — ` +
