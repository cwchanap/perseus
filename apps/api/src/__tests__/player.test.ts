...
describe('player lists (Bun)', () => {
	...
	it('GET stats returns a variant-only row and profile totals from the combined model', async () => {
		...
		const [statsResponse, profileResponse] = await Promise.all([
			buildApp().request('/api/player/stats', { headers: AUTH_COOKIE }),
			buildApp().request('/api/player/profile', { headers: AUTH_COOKIE })
		]);
		...
		expect((await profileResponse.json()).summary).toEqual({
			puzzlesUploaded: 0,
			puzzlesSolved: 1,
			totalCompletions: 2
		});
	...
	it('GET stats forwards limit and v2 cursor query params', async () => {
		...
		await buildApp().request('/api/player/stats?limit=10&cursor=v2%7C1%7C%7Cvariant-only', {
			headers: AUTH_COOKIE
		});
	...
	it('GET stats returns structured 400 for an invalid cursor', async () => {
		...
		const response = await buildApp().request('/api/player/stats?cursor=v3%7C0%7C10%7Cpz1', {
			headers: AUTH_COOKIE
		});
	...
	it('GET stats rejects an explicitly empty cursor', async () => {
		const response = await buildApp().request('/api/player/stats?cursor=', {
			headers: AUTH_COOKIE
		});
	...
	it('GET stats does not mislabel database errors as invalid cursors', async () => {
		...
		const response = await buildApp().request('/api/player/stats', { headers: AUTH_COOKIE });
