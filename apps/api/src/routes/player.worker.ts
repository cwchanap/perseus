...
player.put('/bookmarks/:familyId', requirePlayerAuth, async (c) => {
	...
	const db = getWorkerDbContext(c.env).db;
	// The insert is fenced on the family's deletion tombstone inside the same
	// D1 batch: KV readiness above can race with family deletion (stale KV or
	// a check that ran before deletion started), so the tombstone — not the
	// best-effort ownership row — is the authoritative deleted marker.
	// 'family_missing' means deletion was already fenced — surface the same
	// 404 as the KV check.
