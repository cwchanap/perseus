...
function validatePuzzleMetadataImpl(
	meta: unknown,
	validatePieces: boolean
): meta is PuzzleMetadata {
	...
	if (!m.status || !VALID_PUZZLE_STATUSES.includes(m.status)) return false;
...
export function validatePuzzleMetadata(meta: unknown): meta is PuzzleMetadata {
	return validatePuzzleMetadataImpl(meta, true);
}
...
export function validatePuzzleMetadataLight(meta: unknown): meta is PuzzleMetadata {
	return validatePuzzleMetadataImpl(meta, false);
}
