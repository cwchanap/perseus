...
import { serializeSession, loadPersistedSession } from './codec';
...
function makeGridMetadata(gridCols: number, gridRows: number): SessionPuzzleSpec {
 ...
 */
function contextFromMetadata(metadata: SessionPuzzleSpec): SessionValidationContext {
	return {
...
function makeOptions(
	overrides: Partial<{ clock: Clock; metadata: SessionPuzzleSpec; mode: 'timed' | 'relaxed' }> = {}
) {
	return {
...
describe('PuzzleSession construction metadata validation', () => {
	function baseMetadata(overrides: Partial<SessionPuzzleSpec> = {}): SessionPuzzleSpec {
	...
	it('throws when pieces.length !== pieceCount', () => {
		...
		(metadata as SessionPuzzleSpec).pieces = metadata.pieces.slice(0, 3);
		expect(() =>
			createPuzzleSession({ metadata, runIdFactory: makeRunIdFactory(), clock: new ManualClock() })
