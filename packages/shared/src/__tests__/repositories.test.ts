...
async function insertStoredRun(db: ReturnType<typeof makeDb>['db'], row: StoredRunFixture) {
	await db.run(sql`
...
async function selectRunFacts(db: ReturnType<typeof makeDb>['db']) {
	return db
		.select({
...
describe('repositories', () => {
	...
	it('deletePuzzleStats delegates lifecycle finish for every player row on one puzzle', async () => {
		...
		try {
			...
			await insertStoredRun(context.db, {
				playerId: 'p1',
				runId: 'run-p1-pz1',
				puzzleId: 'pz1',
			...
			expect(await selectRunFacts(context.db)).toEqual([
				{
					playerId: 'p1',
					runId: 'run-p1-pz2',
					puzzleId: 'pz2',
...
describe('player stats', () => {
	...
	it('combines historical baselines and versioned ledger groups without double-counting', async () => {
		...
		await insertStoredRun(helper.db, {
			playerId: 'p1',
			runId: 'standard-1',
			puzzleId: 'standard',
			resultClass: 'standard_timed',
