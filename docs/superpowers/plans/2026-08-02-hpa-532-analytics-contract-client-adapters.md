...
- The ledger retains 90 days and at most 200 entries, newest first.
...
- Context is a snapshot captured at each event's emission time. It is not frozen at
  `puzzle_opened`; viewport, input, progress, result class, rotation, and assistance may legitimately
  differ between events in the same run.
...
- `first_piece_placed.openToPlacementMs` uses integer milliseconds.
  `elapsedActiveSeconds` uses whole seconds. The catalog must call out this mixed-unit contract.
- `puzzle_completed` does not carry `placedPieceCount`; successful completion already guarantees a
  full board and the context progress bucket must be `100`.
- HTTP endpoint selection is configurable. Production normally uses same-origin
  ...
  on the existing CORS configuration.
...
- HPA-532 exports no automatically enabled network client. HPA-534 constructs the application
  client after HPA-533 provides the endpoint.
...
| `packages/types/src/analytics.ts` | V1 inputs/envelopes, bounded dimensions, ID rules, exact validators, and batch contract |
...
| `apps/web/src/lib/services/analytics/context.test.ts` | Piece-count, aspect, viewport, progress, input, auth, content-origin, and assistance boundaries |
| `apps/web/src/lib/services/analytics/run-ledger.ts` | Versioned localStorage ledger and once-per-run mark/prune policy |
| `apps/web/src/lib/services/analytics/run-ledger.test.ts` | Reload idempotency, key tuple, retention, cap, corruption, future schemas, and storage failures |
...
invalid numeric values, deterministic-ID mismatches, non-canonical UUIDs, completion
`placedPieceCount`, and batches outside `1..20`. Add `@ts-expect-error` cases for arbitrary events,
...
Move the existing symbols from `index.ts` into `completion.ts`. Re-export them from `index.ts`.
Do not alter regexes, result eligibility, elapsed-time validation, or exported names.
...
Use explicit exact-key helpers and an event-name switch. UUID occurrence events validate canonical
lowercase UUID v4 IDs. Once-per-run events validate:
...
- Equal image dimensions are square.
...
- Context builders snapshot values at call time.
...
Cover first mark, reload duplicate, all three key components, different events/runs, 90-day
pruning, 200-entry cap, exact validation, corrupt reset, future-schema preservation, and read/write
exceptions.

- [ ] **Step 2: Run the focused browser test and verify failure**
- [ ] **Step 3: Implement latest-read marking, tuple comparison, pruning, and fail-closed behavior**
- [ ] **Step 4: Run focused tests**
- [ ] **Step 5: Commit with `feat(web): add analytics run ledger`**

The ledger is synchronous and marks before enqueue. Cross-tab races remain defensively
deduplicable downstream through deterministic once-per-run event IDs.
...
overflow, latest-20 page-hide behavior, and disposal.
...
- [ ] **Step 1: Write failing facade and compile-time-negative tests**
- [ ] **Step 2: Run the focused test and verify failure**
...
Include exact bucket boundaries, assistance-mode derivation, API-origin fallback, and the rule that
changed fields/boundaries/enum meanings require V2.
...
same-origin/default and local cross-origin/CORS behavior, and HPA-533/HPA-534 ownership.

- [ ] **Step 4: Document privacy**

Record data minimization, excluded fields, run-ID run scope, local ledger retention, and the
production privacy-policy/consent gate. State that this engineering record is not legal advice.
...
- [ ] Assistance `mixed` semantics and time units are explicit.
...
- [ ] HPA-533 can consume `AnalyticsBatchV1` and `isAnalyticsBatchV1` without web imports.
